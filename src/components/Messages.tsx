...
const MessagesImpl = ({
  ...
  const renderableMessages = useMemo(() => {
    ...
    const sliceStart = capApplies
      ? computeSliceStart(collapsed_0, sliceAnchorRef)
      : 0
    return renderRange
  ...
  const isItemExpanded = useCallback(
    (msg_5: RenderableMessage) =>
      expandedKeys.size > 0 && expandedKeys.has(expandKey(msg_5)),
    [expandedKeys],
  )
  ...
  const extractSearchText = useCallback(
    (msg_9: RenderableMessage): string => {
      ...
      if (
        msg_9.type === 'user' &&
        msg_9.toolUseResult &&
        Array.isArray(msg_9.message.content)
      ) {
        ...
        if (tr && 'tool_use_id' in tr) {
          ...
          const extracted = tool_0?.extractSearchText?.(
            msg_9.toolUseResult as never,
          )
